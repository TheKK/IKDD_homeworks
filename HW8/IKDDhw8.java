 ...
 */
public class SimpleCart_entropy
  ...
  /** Successor nodes. */
  protected SimpleCart_entropy[] m_Successors;
  ...
  protected void makeTree(Instances data, int totalInstances, int[][] sortedIndices,
	...
	return;
      ...
      m_isLeaf = false;
      m_Successors = new SimpleCart_entropy[2];
      for (int i = 0; i < 2; i++) {
	m_Successors[i] = new SimpleCart_entropy();
  ...
  public void prune(double alpha) throws Exception {
    ...
    while (prune) {
      ...
      SimpleCart_entropy nodeToPrune = nodeToPrune(nodeList);
...
//	  splitPoint = Math.rint((inst.value(att) + currSplit)/2.0*100000)/100000.0;
   ...
   */
  protected SimpleCart_entropy nodeToPrune(Vector nodeList) {
    if (nodeList.size()==0) return null;
    if (nodeList.size()==1) return (SimpleCart_entropy)nodeList.elementAt(0);
    SimpleCart_entropy returnNode = (SimpleCart_entropy)nodeList.elementAt(0);
    double baseAlpha = returnNode.m_Alpha;
    for (int i=1; i<nodeList.size(); i++) {
      SimpleCart_entropy node = (SimpleCart_entropy)nodeList.elementAt(i);
  ...
  public static void main(String[] args) {
    runClassifier(new SimpleCart_entropy(), args);
  }
}
